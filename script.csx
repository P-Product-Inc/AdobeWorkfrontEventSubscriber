public class Script : ScriptBase
{
    ...
    public override async Task<HttpResponseMessage> ExecuteAsync()
    {
        ...
        await LogTheHttpRequestInputs().ConfigureAwait(false);
    ...
    private async Task LogTheHttpRequestInputs()
    {
        var request = this.Context.Request;
        var headers = new StringBuilder();
        foreach (var header in this.Context.Request.Headers)
        {
            headers.AppendLine(string.Format("{0} ==> {1}", header.Key, string.Join(";", header.Value)));
        }
        this.Context.Logger.LogError("Request's headers:" + headers.ToString());
        headers = new StringBuilder();
        foreach (var header in this.Context.Request.Content.Headers)
        {
            headers.AppendLine(string.Format("{0} ==> {1}", header.Key, string.Join(";", header.Value)));
        }
        this.Context.Logger.LogError("Request's content headers:" + headers.ToString());
        string payload = string.Empty;
        if (request.Content != null)
        {
            await request.Content.LoadIntoBufferAsync().ConfigureAwait(false);
            payload = await request.Content.ReadAsStringAsync().ConfigureAwait(false);
        }
        this.Context.Logger.LogError("Request's payload:" + payload);
    }
